jQuery(document).ready(function($){
                ...
                if(data.message == 'success'){
                    if(easyCooki.v_city == 'yes'){
                        ...
                        $.each(data.outputs, function(k,v){
                            ...
                            output+='<option data-cnt="'+country+'" '+selected+' value="'+v+'">'+v+'</option>';
                        });
                        selectCity.html(output);
                    }else{
                        output+='<option value="">Select Area...</option>';
    ...
    */
    ...
    //Enable State & Country when place order
    jQuery(document.body).on('click', 'button[name="woocommerce_checkout_place_order"]', function(e){
        jQuery('select[name="shipping_country"], select[name="shipping_state"]').prop('disabled', false);
    });


    // Date picker with time
    if(jQuery('#date_slot').length){
        var daterange = [];
        timeslot.slotdates.forEach(function(k, v){
            // console.log(k);
            daterange.push(k);
        });
        ...
        function available(date) {
            ...
            if ($.inArray(dmy, daterange) != -1) {
              return [true, "","Available"];
            } else {
              return [false,"","unAvailable"];
            }
          }
        $('#date_slot').datepicker({
            beforeShowDay: available,
            dateFormat: 'dd-mm-yy',
            onClose: function (selectedDate) {
                console.log('selected: ' + selectedDate);
               console.log('today: ' + timeslot.today);
                // if(selectedDate == timeslot.today && )
                var cutofftime = timeslot.slots.cut_off.split(' ');
                var endtimesolot = (Date.parse(timeslot.today +' '+ timeslot.slots.e_time) < Date.parse(timeslot.today+' '+cutofftime[1]))?timeslot.slots.e_time:cutofftime[1];
                // console.log('distance: ' + dateDiffere);

                jQuery('#time_slot').timepicker({
                    timeFormat: 'h:mm p',
                    interval: 15,
                    minTime: timeslot.slots.s_time,
                    maxTime: endtimesolot,
                    defaultTime: '11',
                    startTime: timeslot.slots.s_time,
                    dynamic: false,
                    dropdown: true,
                    scrollbar: true
                });
        ...
        if(jQuery('.express_time_slot').length){
            jQuery('#exp_time_slot').timepicker({
                timeFormat: 'h:mm p',
                interval: 15,
                maxTime: timeslot.express_cut_time,
                defaultTime: '08',
                dynamic: false,
                dropdown: true,
                scrollbar: true
        ...
        // page reload if chnage shipping method\


        if(jQuery('body.woocommerce-checkout').length){
            var methodname = jQuery('input[name="shipping_method[0]"]:checked').val();
            if(methodname == 'easy_shipping'){
                document.getElementById("exp_time_slot").disabled = true;
                jQuery('.express_time_slot').addClass('hidden');
            }else if(methodname == 'express_delivery'){
                document.getElementById("date_slot").disabled = true;
                document.getElementById("time_slot").disabled = true;
                jQuery('p#date_slot_field, p#time_slot_field').addClass('hidden');
            }
            ...
            jQuery( document.body ).on( 'update_checkout', function(){
                ...
                if(methodname == 'easy_shipping'){
                    document.getElementById("exp_time_slot").disabled = true;
                    jQuery('.express_time_slot').addClass('hidden');
                    jQuery('p#date_slot_field, p#time_slot_field').removeClass('hidden');
                }else if(methodname == 'express_delivery'){
                    document.getElementById("date_slot").disabled = true;
                    document.getElementById("time_slot").disabled = true;
                    jQuery('p#date_slot_field, p#time_slot_field').addClass('hidden');
                    jQuery('.express_time_slot').removeClass('hidden');
                }
